...
function ModulePreTest() {
  ...
  const handleUploadQuestionBank = async (file: File) => {
    ...
    try {
      ...
      await httpService.post("/moduleassessment/create_pretest", formData, {
        headers: {
  ...
  const viewStats = async () => {
    try {
      const { data } = await httpService.get(
        `/moduleassessment/view_pretest_stats/${moduleId}`,
      );
  ...
  const handleViewAssessment = async (assessmentId: string) => {
    try {
      ...
      const response = await httpService.get(
        `/moduleassessment/view_pretest_questions/${assessmentId}`,
      );

      setQuestions(response.data);
    } catch (error) {
      console.error("Failed to fetch pre-test questions:", error);
  ...
  // const handleDeleteAssessment = async (bank: QuestionBank) => {
  //   const confirmed = window.confirm(
  //     `Are you sure you want to delete Question Bank ${bank._id}? This action cannot be undone.`,
  //   );

  //   if (!confirmed) return;

  //   try {
  //     setDeletingAssessment(bank._id);

  //     await httpService.delete(`/moduleassessment/delete_pretest/${bank._id}`);
  ...
  const handleDeleteAssessment = async () => {
    ...
    try {
      ...
      await httpService.delete(
        `/moduleassessment/delete_pretest/${assessmentToDelete._id}`,
      );
        ...
        <section className="mt-8 rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex flex-col gap-4 border-b border-slate-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Question banks available for this module pre-test.
              </p>
            </div>
          ...
          {statistics.banks.length === 0 && (
            <div className="px-6 py-16 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                Upload an Excel question bank to begin building the pre-test for
                this module.
              </p>
